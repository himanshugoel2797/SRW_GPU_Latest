...
{//Moved from derived classes: loops over E, calls derived PropagateRadiationSingleE_Meth_0
 ...
 //It is virtual ("standard" processing). Known re-definitions: in srTDriftSpace
	...
	srTSRWRadStructAccessData** pRadDataSingleE = new srTSRWRadStructAccessData * [nWfr] {0};
	if (pRadDataSingleE == 0) return MEMORY_ALLOCATION_FAILURE;
	srTSRWRadStructAccessData** pPrevRadDataSingleE = new srTSRWRadStructAccessData * [nWfr] {0};
	if (pPrevRadDataSingleE == 0) {
	...
	//separate processing of wavefront radius is necessary
	//HG02012022 Assume these are the same for all wavefronts
	//TODO: Add a check to make sure these are the same for all
	double origRobsX = pRadAccessData->RobsX, origRobsXAbsErr = pRadAccessData->RobsXAbsErr;
	double origRobsZ = pRadAccessData->RobsZ, origRobsZAbsErr = pRadAccessData->RobsZAbsErr;
	double origXc = pRadAccessData->xc; //OC180813
	double origZc = pRadAccessData->zc;
	...
	for (int i = 0; i < nWfr; i++) {
		if (pRadAccessData[i].ne == 1)
		{
			pRadDataSingleE[i] = pRadAccessData + i;
		}
		else
		{
			if (result = SetupNewRadStructFromSliceConstE(pRadAccessData + i, -1, pRadDataSingleE[i])) return result;
			//allocates new pRadDataSingleE !
		}

		if (!m_PropWfrInPlace)
		{
			if (result = SetupNewRadStructFromSliceConstE(pRadAccessData + i, -1, pPrevRadDataSingleE[i])) return result;
		}

		for (int ie = 0; ie < neOrig; ie++)
		{
			if (pRadDataSingleE[i] != pRadAccessData + i)
			{
				if (result = ExtractRadSliceConstE(pRadAccessData + i, ie, pRadDataSingleE[i]->pBaseRadX, pRadDataSingleE[i]->pBaseRadZ)) return result;
				pRadDataSingleE[i]->eStart = pRadAccessData[i].eStart + ie * pRadAccessData[i].eStep;
				long OffsetMom = AmOfMoments * ie;
				pRadDataSingleE[i]->pMomX = pRadAccessData[i].pMomX + OffsetMom;
				pRadDataSingleE[i]->pMomZ = pRadAccessData[i].pMomZ + OffsetMom;
				pRadDataSingleE[i]->RobsX = origRobsX; pRadDataSingleE[i]->RobsXAbsErr = origRobsXAbsErr;
				pRadDataSingleE[i]->RobsZ = origRobsZ; pRadDataSingleE[i]->RobsZAbsErr = origRobsZAbsErr;

				pRadDataSingleE[i]->xc = origXc; //OC180813
				pRadDataSingleE[i]->zc = origZc;

				//if(pRadAccessData->xStart != pRadDataSingleE->xStart) pRadDataSingleE->xStart = pRadAccessData->xStart;
				//if(pRadAccessData->xStep != pRadDataSingleE->xStep) pRadDataSingleE->xStep = pRadAccessData->xStep;
				//if(pRadAccessData->zStart != pRadDataSingleE->zStart) pRadDataSingleE->zStart = pRadAccessData->zStart;
				//if(pRadAccessData->zStep != pRadDataSingleE->zStep) pRadDataSingleE->zStep = pRadAccessData->zStep;
				pRadDataSingleE[i]->xStart = pRadAccessData[i].xStart;
				pRadDataSingleE[i]->xStep = pRadAccessData[i].xStep;
				pRadDataSingleE[i]->zStart = pRadAccessData[i].zStart;
				pRadDataSingleE[i]->zStep = pRadAccessData[i].zStep;
			}
			if (pPrevRadDataSingleE[i] != 0)
			{
				if (result = ExtractRadSliceConstE(pRadAccessData + i, ie, pPrevRadDataSingleE[i]->pBaseRadX, pPrevRadDataSingleE[i]->pBaseRadZ, true)) return result; //OC120908
				pPrevRadDataSingleE[i]->eStart = pRadDataSingleE[i]->eStart;
				pPrevRadDataSingleE[i]->pMomX = pRadDataSingleE[i]->pMomX;
				pPrevRadDataSingleE[i]->pMomZ = pRadDataSingleE[i]->pMomZ;
			}
		}
	}
	...
	for (int i = 0; i < nWfr; i++) 
	{
		...
		for (int ie = 0; ie < neOrig; ie++){
			if (pRadDataSingleE[i] != pRadAccessData + i)
			{
				if (result = UpdateGenRadStructSliceConstE_Meth_0(pRadDataSingleE[i], ie, pRadAccessData + i)) return result;
				//the above doesn't change the transverse grid parameters in *pRadAccessData

				//vRadSlices.push_back(*pRadDataSingleE); //this automatically calls destructor, which can eventually delete "emulated" structs!
				//srTSRWRadStructAccessData copyRadDataSingleE(*pRadDataSingleE); //this doesn't assume to copy pBaseRadX, pBaseRadZ
				srTSRWRadStructAccessData copyRadDataSingleE(*pRadDataSingleE[i], false); //OC290813 fixing memory leak(?) //this doesn't assume to copy pBaseRadX, pBaseRadZ
				copyRadDataSingleE.pBaseRadX = copyRadDataSingleE.pBaseRadZ = 0; copyRadDataSingleE.BaseRadWasEmulated = false;
				vRadSlices.push_back(copyRadDataSingleE);

				if ((pRadDataSingleE[i]->nx != pRadAccessData[i].nx) || (pRadDataSingleE[i]->xStart != pRadAccessData[i].xStart) || (pRadDataSingleE[i]->xStep != pRadAccessData[i].xStep)) gridParamWereModifInSlices = true;
				if ((pRadDataSingleE[i]->nz != pRadAccessData[i].nz) || (pRadDataSingleE[i]->zStart != pRadAccessData[i].zStart) || (pRadDataSingleE[i]->zStep != pRadAccessData[i].zStep)) gridParamWereModifInSlices = true;
			}
		}

		if ((pRadAccessData[i].RobsX != 0) && (pRadAccessData[i].RobsXAbsErr == 0)) pRadAccessData[i].RobsXAbsErr = ::fabs(0.1 * pRadAccessData[i].RobsX);
		if ((pRadAccessData[i].RobsZ != 0) && (pRadAccessData[i].RobsZAbsErr == 0)) pRadAccessData[i].RobsZAbsErr = ::fabs(0.1 * pRadAccessData[i].RobsZ);

		//OCTEST (commented-out)
		if (gridParamWereModifInSlices)
		{//to test!
			if (result = ReInterpolateWfrDataOnNewTransvMesh(vRadSlices, pRadDataSingleE[i], pRadAccessData + i)) return result;
		}
		
		if ((pRadDataSingleE[i] != 0) && (pRadDataSingleE[i] != pRadAccessData + i)) delete pRadDataSingleE[i];
		if ((pPrevRadDataSingleE[i] != 0) && (pPrevRadDataSingleE[i] != pRadAccessData + i)) delete pPrevRadDataSingleE[i];
	}

	delete[] pRadDataSingleE;
	delete[] pPrevRadDataSingleE;
	return 0;
}
