...
//int srTDriftSpace::PropagateRadiationSimple_PropFromWaist(srTSRWRadStructAccessData* pRadAccessData, srTDriftPropBufVars* pBufVars) //OC06092019
//OC01102019 (restored)
int srTDriftSpace::PropagateRadiationSimple_PropFromWaist_Batch(srTSRWRadStructAccessData* pRadAccessData, int nWfr, gpuUsageArg_t* pGpuUsage)
{//Should be very similar to PropagateRadiationSimple_PropToWaist, consider merging
	int result = 0;

	vector<srTDriftPropBufVars> BufVars(nWfr); //OC29082019
	vector<srTDataPtrsForWfrEdgeCorr> DataPtrsForWfrEdgeCorrs(nWfr);
	...
	for (int i = 0; i < nWfr; i++) 
	{
		...
		//SetupPropBufVars_PropFromWaist(pRadAccessData, pBufVars); //OC06092019
		//OC01102019 (restored)
		SetupPropBufVars_PropFromWaist(pRadAccessData + i, BufVars.data() + i);
		//SetupPropBufVars_PropFromWaist(pRadAccessData);
		...
		//OC01102019 (restored)
		BufVars[i].PassNo = 1;
		if (result = TraverseRadZXE(pRadAccessData + i, BufVars.data() + i)) return result;
		//OC06092019
		//pBufVars->PassNo = 1;
		//if(result = TraverseRadZXE(pRadAccessData, pBufVars)) return result;
		...
		//OCTEST (commented-out "edge correction")
		//OC01102019 (uncommented)
		if (result = SetupWfrEdgeCorrData(pRadAccessData, pRadAccessData->pBaseRadX, pRadAccessData->pBaseRadZ, *(DataPtrsForWfrEdgeCorrs.data() + i), pGpuUsage)) return result;
	}
	...
	for (int i = 0; i < nWfr; i++)
	{
		//OCTEST (commented-out "edge correction")
		//OC01102019 (uncommented)
		if (DataPtrsForWfrEdgeCorrs[i].WasSetup)
		{
			MakeWfrEdgeCorrection(pRadAccessData, pRadAccessData->pBaseRadX, pRadAccessData->pBaseRadZ, *(DataPtrsForWfrEdgeCorrs.data() + i));
			DataPtrsForWfrEdgeCorrs[i].DisposeData();
		}

		//Re-scaling
		pRadAccessData[i].xStart = (FFT2DInfo.xStartTr) * LambdaM_Length;
		pRadAccessData[i].zStart = (FFT2DInfo.yStartTr) * LambdaM_Length;
		pRadAccessData[i].xStep = FFT2DInfo.xStepTr * LambdaM_Length;
		pRadAccessData[i].zStep = FFT2DInfo.yStepTr * LambdaM_Length;
