...
int srTGenOptElem::RadResizeCore(srTSRWRadStructAccessData& OldRadAccessData, srTSRWRadStructAccessData& NewRadAccessData, srTRadResize& RadResizeStruct, char PolComp, gpuUsageArg_t* pGpuUsage)
{
	...
	if((!RadResizeStruct.doNotTreatSpherTerm()) && WaveFrontTermCanBeTreated(OldRadAccessData)) //OC090311
	{
		...
		TreatStronglyOscillatingTerm(OldRadAccessData, 'r', PolComp, -1, 0);//pGpuUsage);
	...
	GPU_COND(pGpuUsage,
	{
		printf ("RadResizeCore GPU\n");
		RadResizeCoreParallel(OldRadAccessData, NewRadAccessData, PolComp);
	})
	else
	{
		printf ("RadResizeCore CPU\n");
